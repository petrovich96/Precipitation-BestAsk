...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    message = (
        "Це бот для перевірки ринків Polymarket у категорії Precipitation.\n\n"
        "Команди:\n"
...
async def scan_precipitation_markets(client: PolymarketClient) -> list[EventScanResult]:
    tag = await client.get_tag_by_slug("precipitation")
    tag_id = tag.get("id")
    if tag_id is None:
        raise ValueError("Polymarket did not return a tag id for 'precipitation'.")

    events = await client.list_active_events_for_tag("precipitation", tag_id)
...
async def scan_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.effective_message:
        await update.effective_message.reply_text("Перевіряю активні ринки Precipitation у Polymarket...")

    try:
        results = await scan_precipitation_markets(client)
