...
def build_market_url(event: dict[str, Any], market: dict[str, Any] | None = None) -> str:
    ...
    return "https://polymarket.com/weather/precipitation"
...
def format_scan_results(results: list[EventScanResult]) -> str:
    if not results:
        return (
            "Зараз у категорії Precipitation не знайдено карток, "
            "де сума best ask усіх кошиків більша за 100¢."
        )

    parts = ["Знайдено картки у Precipitation, де сума best ask перевищує 100¢:\n"]
