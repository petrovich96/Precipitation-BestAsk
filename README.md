# Telegram-бот для Polymarket Precipitation

Цей інструмент шукає в Polymarket картки з категорії `Precipitation`, де сума `best ask` усіх кошиків в одній картці більша за `100¢`.
...
- команду `/scan`, яка перевіряє активні ринки `Precipitation` і повертає короткий список знайдених карток.

## Як бот працює

1. Знаходить категорію `precipitation` через Polymarket API.
...
1. У папці проєкту знайдіть файл `.env.example`.
